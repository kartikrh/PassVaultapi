...
const { generateMarketAndRunners, processPredictScoreMarket } = require("../markets");
...
const createCommentaryService = async (request, fastify) => {
  ...
  if (request.body.team1Players && request.body.team1Players.length > 0 &&
        ...
        if (request.body.team1Players && request.body.team1Players.length > 0 &&
          ...
          for (let info of data) {
            ...
            const teamPlayerData = await getAllTeamPlayersByTeamIdAndPlayerIdQuery(
              { playerId: playerData[0].playerId, teamId: playerData[0].teamId },
              fastify, request
            );
...
const updateCommentaryService = async (request, fastify) => {
  ...
  if (
    ...
    (global.tblCommentaries[index].eventRefId != null || global.tblCommentaries[index].eventId != null)
  ) {
    if (
    ...
    if (
      ...
      for (let i = 0; i < TotalInnning; i++) {
        ...
        await updateCommentaryTeams(request, fastify, {
          teamCaptain: request.body.team1Captain ? request.body.team1Captain : null,
        ...
        await updateCommentaryTeams(request, fastify, {
          teamCaptain: request.body.team2Captain ? request.body.team2Captain : null,
    ...
    if (
      ...
      if (request.body.team1Players && request.body.team1Players.length > 0 &&
        ...
        for (let info of data) {
          ...
          const teamPlayerData = await getAllTeamPlayersByTeamIdAndPlayerIdQuery(
            { playerId: palyerData[0].playerId, teamId: palyerData[0].teamId },
            fastify, request
          );
          if (teamPlayerData && teamPlayerData?.jerseyPlayerImage) {
            await updateCommentaryPlayerJerseyImageQuery(
...
const getMatchListByStatus = async (body, request, fastify) => {
  ...
  for (item of body.commentaryData) {
    ...
    let details = {
      ...
      winId: item.winnerId === null || item.winnerId === undefined ? 0 : item.winnerId,
      // rmk: item.rmk || "",
...
const changeShowClientService = async (request, fastify) => {
  ...
  if (commentaryPlayers.length == 0) {
    await updateShowClientQuery({
      isClientShow: false, commentaryId: request.body.commentaryId
    }, request, fastify);
...
const closeCommentaryService = async (request, fastify) => {
  ...
  for (let commentaryId of request.body.commentaryId) {
    ...
    if (index !== -1) {
      ...
      if (comp.isPointTable == true && global.tblCommentaries[index]?.isTest == false) {
        teamPoint.push({
          commentaryId: commentaryId,
          competitionId: global.tblCommentaries[index].competitionId,
    ...
    if (global.tblCommentaries[index] && global.tblCommentaries[index]?.isTest == false) {
      setPlayerHistoryService({
        commentaryId: request.body.commentaryId
      }, request, fastify)
        .catch
