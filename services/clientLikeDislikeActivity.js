...
const checkClientService = async (request, fastify) => {
    ...
    const checkExist = global.tblClient.find(item => item.clientId === getClientId.wrKey && item.isActive === true);
    if (!checkExist) {
        throw new Error(`Client with this id ${getClientId.wrKey} not found!`);
    }
