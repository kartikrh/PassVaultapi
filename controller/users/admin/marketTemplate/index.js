...
const allMarketTypesAndCategories = async (request, reply, fastify) => {
  try {
    const result = await allMarketTypesAndCategoriesService(request ,fastify);
    reply.status(200).send(success(result, 200));
  } catch (err) {
    errorLogger(fastify, err.message, commonPath + "/allMarketTypesAndCategories", request);
    reply.status(200).send(error(err.message, ERROR_CODES.SERVER_ERROR, 200));
  }
};
