...
const getAllEventMarketsQueryV1 = async (fastify, whereCondition = null) => {
  return await fastify.db.query(
    `SELECT
        ...
        json_agg(
            json_build_object(
                'runnerId' , tr."wrRunnerId",
                'runner', tr."wrRunner",
                'line', tr."wrLine",
                'overRate', tr."wrOverRate",
                'underRate', tr."wrUnderRate",
                'backPrice', tr."wrBackPrice",
                'layPrice', tr."wrLayPrice",
                'backSize', tr."wrBackSize",
                'laySize', tr."wrLaySize",
                'runnerLastUpdate', tr."wrLastUpdate",
                'selectionId', tr."wrSelectionId",
                'selectionStatus', tr."wrSelectionStatus",
                'order', tr."wrOrder",
                'teamId', tr."wrTeamId"
            )
        ) as "runners"
    ...
    LEFT JOIN "tblTeams" tt ON tt."wrTeamId" = tem."wrTeamID"
    ${whereCondition ? `WHERE ${whereCondition}` : ""}
    GROUP BY tem."wrID", tc."wrEventName", tc."wrEventDate", tcom."wrCompetition", tet."wrEventType", tt."wrTeamName"
    `,
    {
      type: fastify.db.QueryTypes.SELECT,
    }
