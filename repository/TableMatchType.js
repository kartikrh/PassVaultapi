    ...
    `,
      {
        bind: [
          ...
          data.isMen === undefined ? false : data.isMen,
        ],
        type: fastify.db.QueryTypes.SELECT,
      }
    );
...
const isHistoryChangeInMatchTypeQuery = async (data, request, fastify) => {
  try {
    return await fastify.db.query(
      `
                update "tblMatchTypes" set
            ...
            `,
      {
        bind: [data.isHistory, data.matchTypeId],
      }
    );
  } catch (err) {
    errorLogger(
      fastify,
      err.message,
      "DB ERROR --> repository/TableMatchType.js/isHistoryChangeInMatchTypeQuery",
      request
    );
    throw new Error(err.message);
  }
...
const isMenChangeMatchTypeQuery = async (data, request, fastify) => {
  try {
    return await fastify.db.query(
      `
        UPDATE "tblMatchTypes" SET
      ...
      `,
      {
        bind: [data.isMen, data.matchTypeId],
      }
    );
  } catch (err) {
    errorLogger(
      fastify,
      err.message,
      "DB ERROR --> repository/TableMatchType.js/isMenChangeMatchTypeQuery",
      request
    );
    throw new Error(err.message);
  }
