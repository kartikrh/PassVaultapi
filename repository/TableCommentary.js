  ...
  try {
    ...
    const rows = await fastify.db.query(
      ...
      SELECT 
        ...
        tc."wrCommentaryId" AS "cid",
        tc."wrEventRefId" AS "erid",
        tet."wrEventType" AS "evt",
        tc."wrMatchTypeId" AS "mtid",
        mt."wrMatchType" AS "mt",
        mt2."wrMatchType" AS "hmt",
        COALESCE(co."wrCompetition", '') AS "cmp",
        tc."wrCompetitionId" AS "cmpid",
        tc."wrCurrentInnings" AS "inn",
        tc."wrEventName" AS "enm",
        TO_CHAR(TIMEZONE('Asia/Kolkata', tc."wrEventDate"), 'DD/MM/YYYY') AS "ed",
        TO_CHAR(TIMEZONE('Asia/Kolkata', tc."wrEventDate"), 'HH12:MI:SS') AS "et",
        TO_CHAR(tc."wrEventDate" AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS.MSZ') AS "utc",
        tct3."wrTeamName" AS "twb",
        CASE
          WHEN tc."wrChoseTo" IS NULL THEN NULL
          WHEN tc."wrChoseTo" = 1 THEN 'BAT'
          ELSE 'BOWL'
        ...
        COALESCE(tc."wrTossRmk", '') AS "trk",
        COALESCE(tc."wrCardType", 0) AS "ct",
        COALESCE(CAST(tct1."wrCrr" AS FLOAT), 0) AS "t1crr",
        COALESCE(CAST(tct2."wrCrr" AS FLOAT), 0) AS "t2crr",
        COALESCE(CAST(tct1."wrRrr" AS FLOAT), 0) AS "t1rrr",
        COALESCE(CAST(tct2."wrRrr" AS FLOAT), 0) AS "t2rrr",
        0 AS crr,
        0 AS rrr,
        tc."wrCommentaryStatus" AS "cst",
        COALESCE(tc."wrCommentaryResult", '') AS "res",
        ...
        tc."wrIsVirtual" as "iv",
        tc."wrOnfieldUmpires" as "ofu",
        tc."wrThirdUmpire" as "tum",
        tc."wrMatchReferee" as "mrf",
        tc."wrSession" as "ses",
        tc."wrBallDelay" as "badly",
        tc."wrOverDelay" as "ovdly",
        tc."wrInningDelay" as "indly",
        tc."wrTossDelay" as "todly",
        tc."wrCountryId" as "coid",
        tc."wrVenueId" as "vid",
        tc."wrScoringType" as "sct",
        tc."wrPythonId" as "pyi",
        tc."wrPythonURI" as "pyu",
        tc."wrViews" as "vws",
        tc."wrEventNo" as "eno",
        ...
        tc."wrPitchType" as "pty",
        tc."wrLawnStriping" as "lst",
        tc."wrPitchAge" as "pag",
        tpc."wrBattingCondition" as "btc",
        tpc."wrPitchCondition" as "ptc",
        tpc."wrPaceBowlingCondition" as "pbc",
        tpc."wrSpineBowlingCondition" as "sbc",
        tw."wrWeather" as "wcn",
        tw."wrTemp" as "tmp",
        tw."wrHumidity" as "hum",
        tw."wrVisibility" as "vis",
        tw."wrWindSpeed" as "wsp",
        tw."wrClouds" as "cld"
      FROM "tblCommentaries" tc
      LEFT JOIN "tblTeams" tt1 ON tt1."wrTeamId" = tc."wrTeam1Id" AND tt1."wrIsDeleted" = false
      LEFT JOIN "tblTeams" tt2 ON tt2."wrTeamId" = tc."wrTeam2Id" AND tt2."wrIsDeleted" = false
...
const getAllCommentaryByCompetitionIdQuery = async (competitionId, request, fastify) => {
  try {
    const result = await fastify.db.query(
      `SELECT
        "wrCommentaryId" as "commentaryId",
    ...
    errorLogger(
      fastify,
      error.message,
      "DB ERROR --> repository/TableCommentary.js/getAllCommentaryByCompetitionIdQuery",
      request
    );
    throw new Error(error.message);
  }
