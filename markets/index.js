...
const { updateMarketToDB, sendSocketData, createMarketAndRunner } = require('./utils');
...
const generateMarketAndRunners = async (data,request,fastify)=>{
    ...
    try {
      ...
      // let comTemplate = await getTemplateRunnerQuery(data, request, fastify);
      ...
      if (commentary.commentaryStatus != 1) {
        ...
        if(ignoreMarkets.length > 0){
          whereCondition += ` AND tem."wrMarketTypeCategoryId" NOT IN (${
            ignoreMarkets.join(",")})`;
        }
        // console.log("whereCondition", whereCondition)
        eventMarket = await getExistingEventMarketsQueryV1(fastify, whereCondition);
      ...
      // return eventMarket;
      // generate market by using the template
...
module.exports = { marketGenRunner ,generateMarketAndRunners};